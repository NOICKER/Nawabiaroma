...
export function Navbar() {
    ...
    return (
        <nav className="fixed inset-x-0 top-4 z-30 flex justify-center px-3 sm:px-4 md:top-8 md:px-6">
            <div className="w-full md:hidden">
                <div className="glass-panel relative flex items-center justify-between rounded-[28px] px-4 py-3 sm:px-5">
                    ...
                    <Link
                        className="px-3 text-center text-base font-bold leading-none tracking-[0.2em] text-[var(--color-ink)] sm:text-lg"
            ...
            >
                <div
                    className={`glass-panel pointer-events-auto hidden overflow-hidden rounded-full md:grid md:items-center md:py-[16px] md:transition-[width,padding,box-shadow] md:duration-500 md:ease-[cubic-bezier(0.22,1,0.36,1)] ${
                        isDesktopExpanded ? 'md:w-[860px] md:px-6 lg:w-[980px] lg:px-8' : 'md:w-[118px] md:px-4'
                    }`}
                    style={{
                ...
                >
                    ...
                    <Link
                        className={`relative flex items-center justify-center overflow-hidden text-center transition-[width] duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] ${
                            isDesktopExpanded ? 'w-[18rem]' : 'w-[2.9rem]'
                        }`}
                        to="/"
                    >
                        <span
                            className={`absolute inset-0 flex items-center justify-center whitespace-nowrap text-xl font-bold leading-none text-[var(--color-ink)] transition-[opacity,transform] duration-350 ${
                                isDesktopExpanded ? 'pointer-events-none translate-y-1 scale-95 opacity-0' : 'translate-y-0 scale-100 opacity-100'
                            }`}
                        >
                            <span className="tracking-[0.04em]">NA</span>
                        </span>
                        <span
                            className={`inline-block whitespace-nowrap text-center text-xl font-bold leading-none text-[var(--color-ink)] transition-[opacity,transform,letter-spacing] duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] ${
                                isDesktopExpanded ? 'translate-y-0 tracking-[0.28em] opacity-100' : 'pointer-events-none translate-y-1 tracking-[0.2em] opacity-0'
                            }`}
                        >
                            NAWABI AROMA
                        </span>
                        <span
                            className={`absolute -bottom-2 left-1/2 h-px -translate-x-1/2 bg-[var(--color-primary)] transition-all duration-500 ${
                                activeSection === 'home'
                                    ? isDesktopExpanded
                                        ? 'w-[72%] opacity-75'
                                        : 'w-5 opacity-75'
                    ...
                    <div
                        className={`flex min-w-0 items-center justify-end gap-3 overflow-hidden transition-all duration-400 ease-out ${
                    ...
                    >
                        ...
                        <button
                            aria-expanded={isCartOpen}
                            aria-label="Cart"
                            className={`inline-flex items-center gap-3 rounded-full px-4 py-2 text-[12px] font-medium uppercase tracking-[0.16em] transition-all duration-300 ${
                                isCartOpen
                                    ? 'bg-[var(--color-ink)] text-[var(--color-canvas)] shadow-[0_10px_24px_rgba(0,0,0,0.18)]'
                                    : 'text-[var(--text-muted)] hover:bg-[var(--color-ink)]/6 hover:text-[var(--color-ink)]'
                            }`}
